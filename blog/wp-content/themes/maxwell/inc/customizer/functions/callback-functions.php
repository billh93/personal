...
function maxwell_control_post_content_callback( $control ) {
	...
	if ( $control->manager->get_setting('maxwell_theme_options[post_content]')->value() == 'excerpt' ) :
		return true;
	else :
		return false;
	endif;
...
function maxwell_slider_activated_callback( $control ) {
	...
	if ( $control->manager->get_setting('maxwell_theme_options[slider_blog]')->value() == 1 ) :
		return true;
	elseif ( $control->manager->get_setting('maxwell_theme_options[slider_magazine]')->value() == 1 ) :
		return true;
	else :
		return false;
	endif;
