...
class Maxwell_Magazine_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			sprintf( esc_html__( 'Magazine Posts: 2 Columns (%s)', 'maxwell' ), wp_get_theme()->Name ), // Name
			array( 
				'classname' => 'maxwell-magazine-posts-columns', 
				'description' => esc_html__( 'Displays your posts from two selected categories. Please use this widget ONLY in the Magazine Homepage widget area.', 'maxwell' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_maxwell_magazine_posts_columns', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_maxwell_magazine_posts_columns', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if( !empty( $widget_title ) ) :
			...
			if( $category_id > 0 ) : 
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'maxwell' ), get_cat_name( $category_id ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_date' ); ?>">
				...
				<?php esc_html_e( 'Display post date', 'maxwell' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_author' ); ?>">
				...
				<?php esc_html_e( 'Display post author', 'maxwell' ); ?>
			</label>
		</p>
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_maxwell_magazine_posts_columns', 'widget' );
