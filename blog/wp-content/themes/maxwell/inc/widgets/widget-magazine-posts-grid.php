...
class Maxwell_Magazine_Posts_Grid_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_maxwell_magazine_posts_grid', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_maxwell_magazine_posts_grid', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	...
	function widget_title( $args, $settings ) {
		...
		if( !empty( $widget_title ) ) :
			...
			if( $settings['category'] > 0 ) : 
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'maxwell' ), get_cat_name( $settings['category'] ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('excerpt'); ?>">
				...
				<?php esc_html_e( 'Display post excerpt', 'maxwell' ); ?>
			</label>
		</p>
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_maxwell_magazine_posts_grid', 'widget' );
